...
function subscribe(listener: () => void) {
  ...
  touch.addEventListener("change", listener);
  display.addEventListener("change", listener);
  window.addEventListener("appinstalled", listener);
  window.addEventListener("pageshow", listener);
  return () => {
    touch.removeEventListener("change", listener);
...
function installationMode() {
  if (navigator.maxTouchPoints <= 0 && !window.matchMedia("(any-pointer: coarse)").matches) return "unavailable";

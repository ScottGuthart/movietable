...
- **Ruled hand sheet:** Twelve films as one sheet: a list labelled "Films to rate" with a 1px Hairline border and a Hairline background that shows through 1px gaps, so the Paper cells read as ruled, not carded. Four columns from 1024px, two from 640px; below 640px the same cells become a carousel: one cell at 88% of the sheet's width with the next peeking at the right edge, the 1px Hairline gaps kept between slides inside the same Hairline border. Beneath it, 12px down, two 36px square outline buttons named "Previous film" and "Next film" frame a Faded Ink "3 of 12" in Body with tabular figures that announces politely as the slide changes; a button disables at either end. The carousel is a region named "Films to rate", each slide a group, and ArrowLeft and ArrowRight move it while it has focus, except when a star inside a slide has focus, which keeps its own arrow keys. Each cell has 16px padding and a 12px vertical gap: the title as a Label link with the table's 16px outbound arrow at 40% opacity; a Caption credit line, tabular ("2010 · Christopher Nolan · English"); the genres at Caption weight 400; a Body summary at line-height 1.625 clamped to two lines, all in Faded Ink; then a bottom row with the 32px rating control, five 20px stars 6px apart, on the left and the 28px ghost "Haven't seen" on the right; on the phone the stars are the 36px touch control and the 32px "Haven't seen" wraps beneath them, flush right, when the two do not fit side by side. Cells fade in over 300ms. While the catalogue loads the sheet shows twelve skeleton cells of square Paper Tint blocks pulsing.
...
- **Do** lay the starter hand out as one ruled sheet: a Hairline border and 1px Hairline gaps between Paper cells, four across from 1024px, two from 640px, and a one-cell carousel with Previous, "3 of 12", and Next beneath it below 640px.
